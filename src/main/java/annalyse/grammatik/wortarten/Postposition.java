 ...
 */
@Entity
@NamedQueries({@NamedQuery(name = "Postposition.findAll", query = "SELECT a FROM Postposition a"),
               @NamedQuery(name = "Postposition.findByLexem", query = "SELECT a FROM Postposition a WHERE a.lexem = :lexem")})
public class Postposition extends Wort{
    private static final long serialVersionUID = 1L;
    private final String[] attribute = {"KASUS","UMSTAND"};
    //Die Attribute einer Postposition:
    private Kasus kasus;
    ...
    public static String getNationalClassNeme(){
        if(WAtt.getLanguage()==0)return "Postposition";
        if(WAtt.getLanguage()==1)return "Postpozycja";
        return "Postposition";
    }
